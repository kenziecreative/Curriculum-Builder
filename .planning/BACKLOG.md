...
**Full voiceover scripts for async delivery**
**Source:** Kasie Treadway, post-build feedback + PM clarification answers
**Description:** The pipeline produces scaffolding (facilitator guides, worksheets, activities) but not the actual instructional content. In a flipped classroom model with weekly async guides, this is the most critical missing layer.

PM decision: Full prose voiceover scripts — not talking point outlines. These should be grounded in the weekly written guides and supplemental materials the agent already generates (handouts, worksheets, facilitator notes).

This is a new content generation stage, not a gap flag. The pipeline needs to produce prose written to be recorded against the slide deck for async delivery.

**Scope:** New pipeline stage after `/curriculum:sessions`.

---

### 2. Intake Improvements

**Per-session time expectation at intake**
**Source:** Kasie Treadway, post-build feedback
**Description:** Time math required multiple iteration cycles because the per-session target wasn't established until after the first validation failure. Fix: add intake question "How long should participants spend on each session?" to prevent downstream iteration cycles.
**Scope:** `/curriculum:intake`

---

**Pre-program assessment discovery at intake**
**Source:** Kasie Treadway, post-build feedback + PM clarification answers
**Description:** Business Health Score replaced the proposed pre-program self-assessment — a context-specific correction the builder couldn't anticipate. Fix: ask during intake about existing pre-program assessments or onboarding tools.

PM decision: Business Health Score is Hello Alice's standard pre-program assessment, maintained outside the builder. Baked-in default for Boost Camp builds — agent references it, doesn't generate it.
**Scope:** `/curriculum:intake`

---

**Metaskill enum validation before approval**
**Source:** Kasie Treadway, post-build feedback
**Description:** Mismatch between "Communicating" and "Connecting" surfaced late in validation. Fix: validate skill names against the enum list in the metaskills stage before presenting the activation plan for approval.
**Scope:** `/curriculum:metaskills`

---

### 3. Hello Alice Brand Constants

**Baked-in brand voice, color palette, and Business Health Score for Boost Camp builds**
**Source:** Kasie Treadway, post-build feedback + PM clarification answers
**Description:** Brand voice, color palette, and Business Health Score are fixed constants — not per-project variables. No brand capture system needed. Only intake question: whether there are partner-specific branding considerations for co-branded programs.

PM decision: Tool is Hello Alice-only. Brand voice is a fixed constant, not a variable. No voice-capture step or multi-org brand system needed. Revisit only if the tool extends beyond Hello Alice.

**Hello Alice brand voice:** Confident, Inclusive, Smart, Witty, Liberating. Not clinical instructional design language. Materials should feel like they come from a seasoned guide who's been on the journey, not from a compliance document — direct, a little dry, grounded in real-stakes language.
...
**Case example constraints:** Hypothetical only, never real alumni businesses. Must reflect Hello Alice's core audience: women entrepreneurs, people of color, immigrants, underrepresented founders. Industries: food, beauty, retail, services — not generic tech startup archetypes.

**Content decisions:**
- "Private journal" language: use "Keep in your own notes" or "Keep to yourself" — never "Keep in your private journal" (implies a platform-provided space)
- Session ID hygiene: session IDs (e.g., M-1-S-1) are internal only. Participant-facing materials use week number + lesson title. Facilitator guides may retain session IDs.
- Community vs. private: two-column layout is effective, keep it. "Private" = participant's own notes system, not a platform feature.

**Note — overlap with existing backlog:** The existing "Patterns from Brand Compass" section has a `curriculum-voice.md` item. That item identifies the need for a voice source of truth. This item provides the actual brand constants to encode in it. Cross-reference: do not duplicate.

**Scope:** Reference file creation (encode constants into a Hello Alice brand reference file) + intake logic to detect co-branded program scenarios.

---

### 4. New Deliverables

**Internal Weekly Content Map**
**Source:** Kasie Treadway, post-build feedback + PM clarification answers
**Description:** A Word document serving as a production and facilitation tool (not participant-facing). Per week, it includes: session title, week number, format (async or live webinar), estimated duration, pre-work guide (if any), all handouts and worksheets, full narrated slide outline (every slide titled), post-session assignments and what to bring from prior sessions (for live webinar weeks), and community vs. private reflection prompts.
**Scope:** New generation step after `/curriculum:sessions`.

---

**Participant-Facing Weekly Orientation**
**Source:** Kasie Treadway, post-build feedback
**Description:** A section within the Program Overview handout or a standalone document. Helps participants orient before starting a session. Per week: session title, week number, format, estimated time, plain-language list of what they'll work through, and community vs. private reflection prompts.
**Scope:** New generation step or addition to existing handout generation.

---

**Curriculum Completion Tracker**
**Source:** Kasie Treadway, post-build feedback + PM clarification answers
**Description:** A spreadsheet (Excel/Google Sheets) providing build status visibility for the production team. Columns: Week | Session title | Deliverable name | Deliverable type | Status | Notes. Status values: Existed before this run, Created this run, Needs creation, Needs review/update.

PM decision: Agent pre-populates the status column based on its own assessment of what was built during the run. Hello Alice team does final review and sign-off.
**Scope:** New generation step, likely in the approve or validate stage.

---

### 5. Module-to-Session Schema Fidelity

**Add fidelity check between module and session stages**
**Source:** Kasie Treadway, post-build feedback
**Description:** Social learning and belief-challenging quality scores were low on first pass (5.5 and 6.5) despite good design — the module files weren't capturing the design correctly in the required schema fields. Fix: add a check between the module and session stages to surface schema fidelity issues before they compound downstream.

**Note — overlap with existing backlog:** The existing "Content Fidelity" section covers audit-mode content handling (gap-fill, enrich, hands-off modes and source structure preservation). This is a different problem — schema field quality in the standard (cold-start) path, not content preservation in audit mode. Cross-reference: do not duplicate.
...
**Boost Camp-specific pre-delivery checklist**
**Source:** Kasie Treadway, post-build feedback
**Description:** Nine specific checks to run before handing off any Boost Camp build. These address the concrete issues found in the first run:
...
**Note — overlap with existing backlog:** The existing "Patterns from Brand Compass" section has a "Borrow Brand Verifier pattern" item covering NEEDS markers, outcome ID cross-references, and general completeness checks. These nine checks are Boost Camp-specific and more concrete. Cross-reference: do not duplicate.
...
These are not action items — they are design anchors for future work. Confirmed by PM post-build.

- **Two-column community vs. private layout** is effective — keep it
- **Weekly written guides and supplemental materials** (handouts, worksheets, facilitator notes) are solid generation — the gap is teaching content, not structural quality
- **The iterative correction process works** — even when multiple cycles are needed, the process gets to the right result
- **Pipeline produces usable scaffolding** — the missing layer is instructional content (voiceover scripts), not structure
...
*Last updated: 2026-04-06 — Boost Camp post-build feedback added*
