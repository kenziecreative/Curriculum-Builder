...
**Definition of done:**
1. `/knz-intake` asks fresh vs. existing upfront and routes correctly
...
2. Downstream commands (`/knz-outcomes`, `/knz-assessments`, `/knz-modules`, `/knz-sessions`) detect pre-populated files and switch to compliance-and-enrichment mode rather than generation mode
...
**Success Criteria:**
1. `/knz-evaluate` (or evaluation branch of existing command) accepts curriculum documents and runs them through the three-tier validation schema
...
| 8. Audit Mode Intake | 1/3 | In Progress|  | — |
