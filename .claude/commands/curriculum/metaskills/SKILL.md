...
|---|---|
| `outcome_wording` | Thinking skill activations are mapped to specific learning outcomes — the outcome statements must be current |
| `learner_profile` | Transfer context and contact hours determine how many thinking skills to activate and which transfer prompts to write |

If Stage 6 status is `not-started`, skip this check — there is nothing to be stale against.
...
> **Heads up:** The learning outcomes were updated on {outcome_wording.last_updated} — after Outcomes last ran. The thinking skill map you're about to generate may not reflect the latest outcome changes. You can proceed, or re-run `/curriculum:outcomes` first to pick up the changes.
