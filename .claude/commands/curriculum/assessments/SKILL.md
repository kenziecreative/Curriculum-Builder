...
|---|---|
| `outcome_wording` | Assessment questions and criteria must match the approved learning outcomes exactly |
| `learner_profile` | Skill type, contact hours, and modality determine which assessment formats and duration rules apply |

If Stage 3 status is `not-started`, skip this check — there is nothing to be stale against.
...
> **Heads up:** The learning outcomes were updated on {outcome_wording.last_updated} — after Outcomes last ran. The assessments you're about to generate may not reflect the latest outcome changes. You can proceed, or re-run `/curriculum:outcomes` first to pick up the changes.
